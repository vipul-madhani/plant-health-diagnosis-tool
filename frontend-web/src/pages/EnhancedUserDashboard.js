...
const USER_ID = 1; // TODO: Replace with actual authenticated user ID logic
...
const EnhancedUserDashboard = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const statsResp = await fetch(`${API_URL}/api/stats/user/${USER_ID}`);
      const statsJson = await statsResp.json();
      if (statsJson.success) setUserStats(statsJson.data);
      // Get user plants
      const plantsResp = await fetch(`${API_URL}/api/plants/${USER_ID}`);
      const plantsJson = await plantsResp.json();
      if (plantsJson.success) setPlantDatabase(plantsJson.data);
      // Get activity log
      const actResp = await fetch(`${API_URL}/api/activity/${USER_ID}`);
    ...
    <div className="enhanced-dashboard">
      ...
      <div className="quick-actions">
        ...
        <div className="action-buttons">
          <button className="action-btn"><span className="btn-icon">📱</span><span>Open Mobile App</span></button>
          <button className="action-btn"><span className="btn-icon">👨‍🌾</span><span>Start Consultation</span></button>
          <button className="action-btn"><span className="btn-icon">➕</span><span>Add New Plant</span></button>
          <button className="action-btn"><span className="btn-icon">📊</span><span>View Reports</span></button>
        </div>
      </div>
        ...
        <div className="plants-grid">
          {plantDatabase.map(plant => (
            <div key={plant.id} className="plant-card">
              <div className="plant-image-placeholder"><span className="plant-emoji">🌱</span></div>
              <div className="plant-info">
                <h3>{plant.plant_name}</h3>
                <p className="plant-variety">{plant.variety}</p>
                <p className="plant-date">Planted: {plant.planted_date}</p>
                <div className="health-indicator" style={{ backgroundColor: getHealthColor(plant.health_status) }}>{plant.health_status}</div>
              </div>
          ...
          {recentActivity.map(activity => (
            <div key={activity.id} className="activity-item">
              <div className="activity-icon">
                {activity.activity_type === 'diagnosis' && '🔬'}
                {activity.activity_type === 'consultation' && '💬'}
                {activity.activity_type === 'payment' && '💳'}
                {activity.activity_type === 'plant_added' && '🌱'}
              </div>
              <div className="activity-content">
                <p><strong>{activity.activity_type.charAt(0).toUpperCase()+activity.activity_type.slice(1)}:</strong> {activity.description}</p>
                <span className="activity-date">{activity.created_at}</span>
              </div>
              <div className="activity-status">
                <span className={`status-badge status-${activity.activity_type}`}>{activity.activity_type}</span>
              </div>
            </div>
          ))}
        </div>
